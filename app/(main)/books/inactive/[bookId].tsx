...
import { ConfirmStatusBookModal } from "../../../../components/modals/confirm-delete-book-modal";
...
export default function BookDetailsView() {
  ...
  const [booksFromSameAuthor, setBooksFromSameAuthor] = useState<
    BookProps[] | null
  >(null);
  ...
  return (
    <ScrollView padding="$10" paddingTop="$8" bgColor="$white">
      <Text
        ...
        lineHeight="$4xl"
      >
        {book?.status ? "Inactivar" : "Activar"} libro:{" "}
        {book?.title || "Cargando..."}
      </Text>
      {/* Book title and author */}
      <Text fontSize="$xl" fontWeight="semibold" paddingTop="$2">
        {book?.author || "Cargando..."}
      </Text>
      {/* Book Properties */}
      <Box paddingTop="$3">
        ...
        <Box display="flex" flexDirection="row" alignItems="center" gap="$2">
          ...
          <Text fontSize="$lg" paddingTop="$2">
            {new Date(book?.createdAt as string).toLocaleDateString("es-MX") ||
              "Cargando..."}
          </Text>
        </Box>
        {book?.updatedAt && (
          <Box display="flex" flexDirection="row" alignItems="center" gap="$2">
            ...
            <Text fontSize="$lg" paddingTop="$2">
              {new Date(book?.updatedAt as string).toLocaleDateString(
                "es-MX"
              ) || "Cargando..."}
            </Text>
          </Box>
      ...
      >
        ...
        >
          <ButtonText>{book?.status ? "Inactivar" : "Activar"}</ButtonText>
        </Button>
      </ButtonGroup>
      <ConfirmStatusBookModal
        data={book}
