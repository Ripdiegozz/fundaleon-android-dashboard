...
export function SearchCustomerModal () {
    ...
    const isModalOpen = isOpen && type === 'searchCustomer';
    ...
    const redirectTo = async () => {
      ...
      if (inputType === 'Cédula') {
        try {
          ...
          onClose();
          return router.push(`/customers/details/${res.data.data.id}`);
        } catch (error) {
          console.log(error);
          toast.show({
            placement: "top",
      ...
      <Center>
        ...
        >
          ...
          <AlertDialogContent padding='$4'>
            <AlertDialogHeader>
              <Heading size='lg'>Buscar un cliente</Heading>
              <AlertDialogCloseButton>
            ...
            <AlertDialogBody>
              <Text size='sm'>
                Ingresa la cédula del cliente que deseas buscar. Si no la conoces, puedes buscarlo por email.
              </Text>
            </AlertDialogBody>
