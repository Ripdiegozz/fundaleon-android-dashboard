...
export function SearchEditCustomerModal () {
    ...
    const redirectTo = async () => {
      ...
      if (inputType === 'Cédula') {
        try {
          ...
          onClose();
          return router.push(`/customers/edit/${res.data.data.id}`);
        } catch (error) {
          console.log(error);
          toast.show({
            placement: "top",
      ...
      <Center>
        ...
        >
          ...
          <AlertDialogContent padding='$4'>
            <AlertDialogHeader>
              <Heading size='lg'>Editar un cliente</Heading>
              <AlertDialogCloseButton>
            ...
            <AlertDialogBody>
              <Text size='sm'>
                Ingresa la cédula del cliente que deseas editar. Si no la conoces, puedes buscarlo por email.
              </Text>
            </AlertDialogBody>
